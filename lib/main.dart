...
class MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: FirstRoute(camera: widget.camera),
    );
  }
}
...
class FirstRoute extends StatelessWidget {
  ...
  const FirstRoute({super.key, required this.camera});
            ...
            Center(
              child: ElevatedButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => SecondRoute(camera: camera)),
                  );
                },
              ),
            ),
...
class SecondRoute extends StatefulWidget {
  ...
  const SecondRoute({super.key, required this.camera});
...
class _SecondRouteState extends State<SecondRoute> {
  ...
  Future<void> _initializeTFLite() async {
    try {
      _interpreter = await Interpreter.fromAsset('posenet.tflite');
  ...
  void _startVideoStream() async {
    if (!_cameraController.value.isStreamingImages) {
      await _cameraController.startImageStream((CameraImage img) {
        if (!_isProcessing) {
          _isProcessing = true;
          _processCameraImage(img).then((_) {
            _isProcessing = false;
          });
        }
      });
  ...
  List<double> _convertCameraImageToInputImage(CameraImage image) {
    final img.Image? convertedImage = _convertYUV420toImage(image);
    if (convertedImage == null) return [];

    final resizedImage = img.copyResize(convertedImage, width: 257, height: 257);
    final inputImage = Float32List(257 * 257 * 3);

    for (int y = 0; y < 257; y++) {
      for (int x = 0; x < 257; x++) {
        final pixel = resizedImage.getPixel(x, y);
        inputImage[(y * 257 + x) * 3 + 0] = img.getRed(pixel) / 255.0;
        inputImage[(y * 257 + x) * 3 + 1] = img.getGreen(pixel) / 255.0;
        inputImage[(y * 257 + x) * 3 + 2] = img.getBlue(pixel) / 255.0;
      }
    }
  ...
  img.Image? _convertYUV420toImage(CameraImage image) {
    ...
    final yuvImage = img.Image(width, height);

    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        final yValue = yBuffer[y * image.planes[0].bytesPerRow + x];
        final uValue = uBuffer[(y ~/ 2) * image.planes[1].bytesPerRow + (x ~/ 2)];
        final vValue = vBuffer[(y ~/ 2) * image.planes[2].bytesPerRow + (x ~/ 2)];

        final r = yValue + 1.402 * (vValue - 128);
        final g = yValue - 0.344136 * (uValue - 128) - 0.714136 * (vValue - 128);
        final b = yValue + 1.772 * (uValue - 128);

        yuvImage.setPixelRgba(x, y, r.toInt(), g.toInt(), b.toInt());
      }
    }
  ...
  List<double> _runInference(List<double> inputImage) {
    final output = List<double>.filled(17 * 3, 0.0); // 17 keypoints with (x, y, confidence)
  ...
  void _processOutput(List<double> output) {
    ...
    for (int i = 0; i < 17; i++) {
      final x = output[i * 3 + 1] * _cameraController.value.previewSize!.width;
      final y = output[i * 3 + 0] * _cameraController.value.previewSize!.height;
